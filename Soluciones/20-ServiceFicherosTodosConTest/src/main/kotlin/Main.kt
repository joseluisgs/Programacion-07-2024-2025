...
fun main() {
    ...
    val service = ProductoServiceImpl(
        //storage = ProductosStorageImpl(),
        // repository = ProductosRepositoryImpl(),
        // cache = CacheImpl(CACHE_SIZE, CacheImpl.CacheType.LRU)
    )
    
    // Importar los productos
    val productosInputFile = Path.of(Config.configProperties.dataDir, "products.dat")
    service.importFromFile(productosInputFile.pathString, FileFormat.DAT)
    ...
    // Exportamos los productos
    val productosOutputFile = Path.of(Config.configProperties.backupDir, "products_back.json")
    service.exportToFile(productosOutputFile.pathString, FileFormat.JSON)
