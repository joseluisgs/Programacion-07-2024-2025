...
fun main() {
    ...
    val service = ProductoServiceImpl(
        storageCsv = ProductosStorageCsv(),
        storageJson = ProductosStorageJson(),
        storageXml = ProductosStorageXml(),
        repository = ProductosRepositoryImpl()
    )
    
    // Importar los productos desde un Json
    service.importFromFile("data/products.csv", ExportFormat.CSV)
    ...
    service.exportToFile("data/products_back.xml", ExportFormat.XML)
