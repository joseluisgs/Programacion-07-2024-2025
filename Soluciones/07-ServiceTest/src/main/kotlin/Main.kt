...
fun main() {
    ...
    // como los hemos puesto una implementación por defecto en el costructor podemos ignorar el creralos
    val service = ProductoServiceImpl(
        repository = ProductosRepositoryImpl(),
        storage = ProductosStorageCsv(),
        cache = CacheImpl<Int, Producto>(5, CacheImpl.CacheType.LRU)
    )
    ...
    //productos.map { it.nombre }.forEach { println(it) }
    ...
    println("Media de precio por proveedor, siempre y cuando esa media sea mayor a 10:")
    val proveedorPrecioMedioMayor =
        productos.groupBy { it.idProveedor }
            .mapValues { it.value.map { it.precioUnidad }.average().roundTo(2) }
            .filter { it.value > 10 }
    proveedorPrecioMedioMayor.forEach { println("Proveedor: ${it.key} - Media de precio: ${it.value}") }
    ...
    val precioMaximoMinimoMedioPorCategoria = productos.groupBy { it.idCategoría }.mapValues {
        ...
        val cantidad = it.value.count()
        //Triple(precioMaximo, precioMinimo, precioMedio)
        val res = listOf(precioMaximo, precioMinimo, precioMedio, cantidad)
        res
    }
    ...
    precioMaximoMinimoMedioPorCategoria.forEach {
        println(
            "Categoría: ${it.key} - Precio máximo: ${it.value[0]} - Precio mínimo: ${it.value[1]} - Precio medio: ${it.value[2]} - Cantidad: ${it.value[3]}"
        )
    }
