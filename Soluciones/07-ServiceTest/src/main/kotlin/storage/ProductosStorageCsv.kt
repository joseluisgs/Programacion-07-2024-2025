...
class ProductosStorageCsv : ProductosStorage {
    ...
    override fun readFromFile(file: File): List<Producto> {
        ...
        if (!file.exists() || !file.isFile || !file.canRead()) {
            ...
            throw ProductosException.ProductosStorageException("El fichero no existe, o no es un fichero o no se puede leer. Fichero: ${file.absolutePath}")
        }
    ...
    override fun writeToFile(file: File, productos: List<Producto>) {
        ...
        if (!file.parentFile.exists() || !file.parentFile.isDirectory) {
            ...
            throw ProductosException.ProductosStorageException("El directorio padre del fichero no existe. Fichero: ${file.parentFile.absolutePath}")
        }
        ...
        productos.forEach {
            file.appendText("${it.id},${it.nombre},${it.idProveedor},${it.idCategoría},${it.cantidadPorUnidad},${it.precioUnidad},${it.unidadesEnStock},${it.unidadesEnPedido},${it.nivelDeReabastecimiento},${if (it.descontinuado) 1 else 0}\n")
        }
    }
}